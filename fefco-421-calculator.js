...
const PROFIT_MARGIN_SMALL = 0.80; // For quantity < 101
const PROFIT_MARGIN_MEDIUM = 0.76; // For quantity 101-250
const PROFIT_MARGIN_LARGE = 0.82; // For quantity > 250
...
// Quantity thresholds
const QUANTITY_THRESHOLD_1 = 101;
...
function getBasePrice1(dimensions, quantity) {
    const getBasePrice1 = (((((((dimensions.length * 6) + (dimensions.width * 14) + (dimensions.height * 17)) + 0.5) * 30.5)) * 1.2));

    if (getBasePrice1 < 170) {
        return 215;
    }
...
function getQuantumModifier(quantity) {
    ...
    if (quantity > 499) modifier += 50 / quantity;
    if (quantity > 999) modifier += 75 / quantity;
